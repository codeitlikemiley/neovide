local custom_attach = function(_, bufnr)
  ...
  vim.keymap.set("n", "<F4>", ":lua require(\"neotest\").summary.toggle()<CR>", { silent = true, desc = "Toggle Test Summary" })
  ...
  vim.keymap.set("n", "<leader>rl", ":lua require(\"neotest\").summary.toggle()<CR>", { silent = true, desc = "Test Summary" })
  ...
  vim.keymap.set('n', '<leader>ru', ":lua require('dapui').toggle(); vim.cmd('Neotree position=right')<CR>", { noremap = true, silent = true, desc = "Toggle Debugger UI" })
  vim.keymap.set("n", "<leader>rs", ':execute "RustStartStandaloneServerForBuffer" | LspStop<CR>', { silent = true, desc = "Rust Standalone Server" })
end
